  ...
  private def auditPostRestore(
    ...
    try {
      ...
      if (filesToCheck.isEmpty) {
        ...
        val summary = jsc.parallelize(filesToCheck, Math.max(1, rollbackParallelism))
          .map { pathStr =>
            val p = new Path(pathStr)
            try {
              val exists = HadoopFSUtils.getFs(p, storageConf.unwrap()).exists(p)
              if (exists) AuditSummary(presentCount = 1, samplePresent = Seq(pathStr))
              else AuditSummary(absentCount = 1)
            } catch {
              case e: Exception =>
                AuditSummary(indeterminateCount = 1, sampleIndeterminate = Seq(s"$pathStr: ${e.getMessage}"))
            }
          }
          .treeAggregate(AuditSummary())(
            seqOp = (acc, s) => acc.merge(s),
            combOp = (a, b) => a.merge(b)
          )

        if (summary.presentCount > 0) {
          logError(s"Restore audit FAILED for instant ${restoreInstant.requestedTime()}: " +
            s"${summary.presentCount} file(s) still present, e.g. ${summary.samplePresent.mkString(", ")}")
          "FAILED"
        } else if (summary.indeterminateCount > 0) {
          logWarning(s"Restore audit INCONCLUSIVE for instant ${restoreInstant.requestedTime()}: " +
            s"${summary.indeterminateCount} file(s) had IO errors, e.g. ${summary.sampleIndeterminate.mkString(", ")}")
          "INCONCLUSIVE"
        } else {
          logInfo(s"Restore audit PASSED for instant ${restoreInstant.requestedTime()}: " +
            s"${summary.absentCount} file(s) confirmed absent.")
          "PASSED"
        }
      }
    } catch {
...
object RestoreToInstantProcedure {
  ...
  // Aggregated audit summary. Defined at object level so the generated bytecode does not embed
  // a synthetic outer-class reference; otherwise Spark's ClosureCleaner cannot serialize the
  // closure that returns these values from executors.
  private val MAX_SAMPLES = 5

  private case class AuditSummary(
      presentCount: Long = 0,
      indeterminateCount: Long = 0,
      absentCount: Long = 0,
      samplePresent: Seq[String] = Seq.empty,
      sampleIndeterminate: Seq[String] = Seq.empty) extends Serializable {
    def merge(other: AuditSummary): AuditSummary = AuditSummary(
      presentCount + other.presentCount,
      indeterminateCount + other.indeterminateCount,
      absentCount + other.absentCount,
      (samplePresent ++ other.samplePresent).take(MAX_SAMPLES),
      (sampleIndeterminate ++ other.sampleIndeterminate).take(MAX_SAMPLES)
    )
  }
}
