...
public class HoodieCopyOnWriteTable<T extends HoodieRecordPayload> extends HoodieTable<T> {
  ...
  @Override
  public HoodieWriteMetadata write(JavaSparkContext jsc, String instantTime, List<HoodieWriteOperation<T>> operations) {
    JavaRDD<WriteStatus> writeStatuses = null;
    long indexUpdateDurationSec = 0;
    Map<String, List<String>> partitionToReplaceFileIds = new HashMap<>();

    for (int index = 0; index < operations.size(); ++index) {
      HoodieWriteOperation<T> operation = operations.get(index);
      HoodieWriteMetadata writeMetadata;
      switch (operation.operationType()) {
        case BULK_INSERT:
          writeMetadata = bulkInsert(jsc, instantTime, operation.getRecords(), operation.getBulkInsertPartitioner());
          break;
        case BULK_INSERT_PREPPED:
          writeMetadata = bulkInsertPrepped(jsc, instantTime, operation.getRecords(), operation.getBulkInsertPartitioner());
          break;
        case DELETE:
          writeMetadata = delete(jsc, instantTime, operation.getDeleteKeys());
          break;
        case INSERT:
          writeMetadata = insert(jsc, instantTime, operation.getRecords());
          break;
        case INSERT_OVERWRITE:
          writeMetadata = insertOverwrite(jsc, instantTime, operation.getRecords());
          break;
        case INSERT_PREPPED:
          writeMetadata = insertPrepped(jsc, instantTime, operation.getRecords());
          break;
        case UPSERT:
          writeMetadata = upsert(jsc, instantTime, operation.getRecords());
          break;
        case UPSERT_PREPPED:
          writeMetadata = upsertPrepped(jsc, instantTime, operation.getRecords());
          break;
        default:
      ...
      if (writeStatuses == null) {
        ...
        writeStatuses = writeStatuses.union(writeMetadata.getWriteStatuses());
      }
