...
public class TestMarkerBasedRollbackUtils {
  ...
  public void testGetAllMarkerPathsThrowsIOExceptionOnTransientHdfsFailure() throws IOException {
    ...
    HoodieEngineContext mockContext = mock(HoodieEngineContext.class);
    HoodieStorage mockStorage = mock(HoodieStorage.class);

    when(mockTable.getMetaClient()).thenReturn(mockMetaClient);
    when(mockTable.getStorage()).thenReturn(mockStorage);
    when(mockMetaClient.getBasePath()).thenReturn(new StoragePath(basePath));
    when(mockMetaClient.getMarkerFolderPath(instant)).thenReturn(markerDir);

    // MARKERS.type is absent — simulates a write interrupted before writing the type file
    when(mockStorage.exists(new StoragePath(markerDir, MARKER_TYPE_FILENAME))).thenReturn(false);
    // The marker directory itself exists (the write had started creating marker files)
    when(mockStorage.exists(new StoragePath(markerDir))).thenReturn(true);
    // Listing the marker dir fails transiently (e.g., HDFS namenode throttling)
    when(mockStorage.listDirectEntries(any(StoragePath.class)))
