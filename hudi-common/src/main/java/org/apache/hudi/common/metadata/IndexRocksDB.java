...
public class IndexRocksDB extends Index {
  private static final String COL_FAMILY_NAME = "metadata";
  private RocksDBDAO rocksDBDAO;
  private static byte[] KEY_METADATA = {0};

  public IndexRocksDB(Configuration conf, Path path) {
    super(conf, path);
  }

  @Override
  public void create(Option<Map<String, String>> metadata) throws IOException {
    rocksDBDAO = new RocksDBDAO("default", path.toString());
    rocksDBDAO.addColumnFamily(COL_FAMILY_NAME);
    rocksDBDAO.put(COL_FAMILY_NAME, KEY_METADATA, serializeMetadata(metadata));
  }
  ...
  public void put(byte[] key, byte[] value) throws IOException {
    ...
    rocksDBDAO.put(COL_FAMILY_NAME, key, value);
  }
