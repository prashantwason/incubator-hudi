...
>>>>>>> [HUDI-684] Introduced abstraction for writing and reading different types of base file formats.
  ...
  private void init() throws IOException {
    ...
    if (schemaFromLogFile == null) {
      writerSchema =  readSchema(jobConf, split.getPath());
      LOG.debug("Writer Schema From Base File => " + writerSchema.getFields());
    } else {
      writerSchema = schemaFromLogFile;
      LOG.debug("Writer Schema From Log File => " + writerSchema.getFields());
    }
