 ...
 */
public class HoodieRealtimeInputFormat extends HoodieInputFormat {
  ...
  public HoodieRealtimeInputFormat(FileInputFormat baseInputFormat) {
    super(baseInputFormat);
  }
  ...
  public RecordReader<NullWritable, ArrayWritable> getRecordReader(final InputSplit split, final JobConf jobConf,
    ...
    if (jobConf.get(HOODIE_READ_COLUMNS_PROP) == null) {
      synchronized (jobConf) {
        ...
        if (jobConf.get(HOODIE_READ_COLUMNS_PROP) == null) {
          ...
          conf = jobConf;
          conf.set(HOODIE_READ_COLUMNS_PROP, "true");
        }
      }
    }
    ...
    return new HoodieRealtimeRecordReader((HoodieRealtimeFileSplit) split, jobConf,
       baseInputFormat.getRecordReader(split, jobConf, reporter));
  }

  /**
   * Add a field to the existing fields projected.
   */
  private Configuration addProjectionField(Configuration conf, String fieldName, int fieldIndex) {
  ...
  private void addRequiredProjectionFields(Configuration configuration) {
   ...
   */
  public void cleanProjectionColumnIds(Configuration conf) {
