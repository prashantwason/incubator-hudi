...
public class HoodieShadowPipeline {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // Determine the datacenter based on spark configs
    final String datacenter = jssc.getConf().get("spark.driver.host").contains("phx") ? "phx" : "dca";

    // Rest of the operations need to be under a lock to prevent conflicts
    try (LockManager lockManager = new LockManager(createWriteConfig(cfg, datacenter), srcMetaClient.getStorage())) {
  ...
  private static HoodieWriteConfig createWriteConfig(HoodieShadowPipelineConfig cfg, String datacenter) {
    ...
    if (datacenter.toLowerCase().startsWith("dca")) {
      lockConfigBuilder.withZkQuorum(ZOOKEEPER_CONNECT_URL_DCA);
    } else if (datacenter.toLowerCase().startsWith("phx")) {
      lockConfigBuilder.withZkQuorum(ZOOKEEPER_CONNECT_URL_PHX);
    }
