...
public class HoodieSource<T> extends FileIndexReader implements Source<T, HoodieSourceSplit, HoodieSplitEnumeratorState> {
  ...
  private SplitEnumerator<HoodieSourceSplit, HoodieSplitEnumeratorState> createEnumerator(
    ...
    // Streaming keeps per-subtask assignment (DefaultHoodieSplitProvider) so a file id's
    // incremental splits stay affine to one reader. Bounded backfill instead uses a global
    // work-stealing pool: the full split set is known up front and each split is independent and
    // order-free, so any reader can read any split. Serving from one shared pool keeps every reader
    // busy until it is drained, which removes the declining straggler tail and the mid-run
    // imbalance dips that per-subtask (count-balanced, non-stealing) assignment produces.
